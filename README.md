...
> **🧭 双版本路线（进行中，见 `finetune/INTEGRATION.md`）**
> 现有管线定位为**精翻版**（ASR → LLM 质检/摘要/翻译/审校，术语表+审校兜底）。
> 实验线已验证**快速直出版**：LoRA 微调后的 Qwen3-ASR 用 context 任务开关直接
> 「听日语、写中文」，全程无 LLM——真实 bilibili 视频实测假名占比 0%、CPU RTF
> 0.69~0.91、可省 ~1.8GB 的 1.7B 模型；代价是无术语表时专名漂移、误译无闸可拦。
> 微调/评测/端到端实验记录见 `finetune/README.md` §10–11，推理落地设计（官方
> decoder.int8.onnx 权重补丁路线 + 三道验证闸）见 `finetune/INTEGRATION.md`。
...
- **🔒 完全离线**：识别、质检、翻译全部本地完成。
- **🧠 逐句质检（保守优先）**：每句 ASR 结果都经 LLM 结合上文判定 keep / fix / drop：
...
| `--qc-keep-min-chars N` | 12 | 长句保护：≥N 字且非重复文本的句子不允许被 QC 丢弃（0=关闭） |
| **翻译与排版** | | |
