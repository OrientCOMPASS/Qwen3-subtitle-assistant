...
### 复测：1.7B 适配器（同两个视频；训练 run 36254711659，e2e runs 36261430221 / 36261382151）

训练同款配方（120 对 + 20 静音，r=32，4 epoch，纯 CPU 88 分钟，loss 0.86→**0.59**），
FLEURS 20 条评测：相似度均值 0.54→**0.674**，≥0.8 从 2/20→**6/20**，大偏差从 7/20→**1/20**，
0.6B 的灾难性误译全部消失（修道院→**集体墓地**✓、贝特→**美元**✓、伊斯坦布尔火堆→**复活节日出**✓）。

| 真实视频 e2e | 视频一（0.6B→1.7B） | 视频二（0.6B→1.7B） |
|---|---|---|
| 五项判定 | 过 → **过** | 过 → **过** |
| 假名/汉字占比 | 0.0%/82.2% → 0.0%/82.6% | 0.0%/83.4% → 0.0%/83.3% |
| 空输出丢弃率 | 9% → **0%** | 3% → **0%** |
| RTF（fp32 sidecar 评测运行时） | 0.91 → 1.99 | 0.79 → 1.74 |

内容级结论（20s 桶三方并排，报告在各自 run 的 verdict-report.txt）：

* **视频一（塞翁失马叙事）**：1.7B 把故事核心全部讲对且前后一致（马跑了/马带回名马/
  独子摔腿免兵役/称谓「爷爷」统一），个别段落比 master 精翻还干净（精翻把「更好的马」
  译成「更美味的马」）；0.6B 同段落是乌鸦/乌龟/修道院级漂移；
* **视频二（特摄演讲，术语密集）**：0.6B 的「特撮→特约/特杀」「変身→特发性脊柱炎」
  在 1.7B **基本修复**（特摄/特摄英雄/变身全程一致）；残留差距两类——指代漂移
  （父亲↔叔叔）与**世界知识纠错**（音频把 インフルエンサー 读得像 インフルエンザ，
  master 的 LLM 靠全局语境纠成「网红」，1.7B 忠实译成「流感」）；
* 含糊口音段三方同样翻车——garbage in garbage out，与管线结构无关。

**判定：快速字幕场景下，1.7B 直出足以取代「ASR→LLM 四段」双模型工作流**
（省 1.8GB GGUF 与全部 LLM 轮次）；发布级精翻保留现有管线为可选路径。
遗留工程问题：1.7B ONNX 包 2.24 GiB，超过 GitHub Release 单文件 2 GiB 上限
（decoder 单体 1.90 GiB）——瘦身可行性由 Gate 1-1.7B 探针的 dtype 构成分析回答。
